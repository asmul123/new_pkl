...
class Pembimbing extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['skema'] = $this->Scheme_model->getSchemeThisMentor($biodata->mentor_id, '1');
	...
	public function biodata()
	{
		...
		$header['name'] = $biodata->name;
		$header['role'] = $biodata->role;
		$header['title'] = "Biodata Pembimbing - Jurnal PKL Online SMKN 1 GARUT";
		$header['menuactive'] = "biodata";
		$data['biodata'] = $biodata;
		$this->load->view('templates/header', $header);
		$this->load->view('templates/sidebar');
		$this->load->view('templates/navbar');
		$this->load->view('pembimbing/biodata', $data);
		$this->load->view('templates/footer');
	}
	...
	public function terimapresence($presenceID)
	{
		$this->load->model('Presensi_model');
		$biodata = $this->Mentor_model->getBioMentor($this->session->userdata('email'));
		$cekpresence = $this->Presensi_model->cekPresensceMentor($biodata->mentor_id, $presenceID);
		if ($cekpresence != 0) {
			$data = [
				'status' => '2'
			];
	...
	public function tolakpresence($presenceID)
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->load->view('templates/sidebar');
			$this->load->view('templates/navbar');
			$this->load->view('mentor/tolak_presensi', $data);
			$this->load->view('templates/footer');
		} else {
	...
	public function jurnalproses($jurnalID)
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->load->view('templates/sidebar');
			$this->load->view('templates/navbar');
			$this->load->view('mentor/proses_jurnal', $data);
			$this->load->view('templates/footer');
		} else {
			$noted = $this->input->post('noted');
			$status = $this->input->post('status');
			$cekjurnal = $this->Jurnal_model->cekThisJurnalMentor($biodata->mentor_id, $jurnalID);
			if ($cekjurnal != 0) {
				$data = [
					'status' => $status,
	...
	public function terimaskema($skemaID)
	{
		$this->load->model('Scheme_model');
		$biodata = $this->Mentor_model->getBioMentor($this->session->userdata('email'));
		$cekscheme = $this->Scheme_model->cekSchemeMentor($biodata->mentor_id, $skemaID);
		if ($cekscheme != 0) {
			$data = [
				'status' => '2'
			];
	...
	public function update()
	{
		...
		if ($biodata) {
			if (isset($_POST['image'])) {
				...
				$photo = 'p' . $biodata->mentor_id . time() . '.png';
				...
				$this->db->where('mentor_id', $biodata->mentor_id);
				$this->db->update('mentors');
				redirect(base_url('pembimbing/biodata'));
			} else {
